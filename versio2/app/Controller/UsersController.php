...
class UsersController extends AppController {
   ...
   // Common stuff
   public $uses = array('User', 'Player', 'Club');
      ...
      else // No logged in user
      {
         ...
         if ( $this->request->is('post') ) 
         {
            ...
            if ($this->User->save($data)) 
            {
               ...
               if( $data['User']['role'] == 1 )
               {
                  $playerData['Player'] = $this->request->data['Player'];
                  
                  $playerData['Player']['firstname'] = $data['User']['firstname'];
                  $playerData['Player']['lastname'] = $data['User']['lastname'];
                  $playerData['Player']['email'] = $data['User']['email'];
                  
                  $playerData["User"] = array( 
                     "id" => $id
                  );
                  
                  $this->Player->save($playerData);
   ...
   function deletePlayer($id)
   {
      ...
      if( isset( $player['Player']['id'] ) )
      {
         if( $this->Player->delete( $id ) )
         {
            $this->Session->setFlash( __("Pelaaja poistettu") );
         }
         else
         {
            $this->Session->setFlash( __("Pelaajan poisto epäonnistui") );
         }
